    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Extract odds data (simplified for demo - in real implementation, parse actual odds)
                sample_data = {
                    "race_info": {
                        "race_date": race_date,
                        "venue": venue,
...
async def main():
    ...
    try:
        ...
        if not first_race_data:
            print("❌ Could not get current race information")
            return
        ...
        if not race_date or not venue:
            print("❌ Could not extract race date and venue from current race")
            return
        
        venue_name = "Sha Tin" if venue == "ST" else "Happy Valley"
        print(f"✅ Current race: {race_date} {venue} ({venue_name})")
